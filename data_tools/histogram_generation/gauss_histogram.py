...
def gauss(config: TrainConfig):
    ...
    n_Sig_Pois = np.random.poisson(lam = config.train__signal_number_of_events * np.exp(normalization_factor), size = 1)[0] if config.train_gauss__is_poisson_fluctuations else config.train__signal_number_of_events
    ...
    signal = np.random.multivariate_normal(mean = config.train__signal_location * np.ones(dim), cov = config.train__signal_scale * np.ones((dim, dim)), size = n_Sig_Pois)
