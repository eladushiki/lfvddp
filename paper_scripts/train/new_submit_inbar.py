...
def main():
    ...
    try:
        ...
        if user=="inbarsav":
            ...
            configsdir = scriptsdir
        else:  # Example how Elad added his parameters
        ...
        for sig in number_of_signals:
            ...
            for i in range(len(train_sizes)):
                ...
                for jobid in range(N_jobs):
                    ...
                    cmd_TAU=f"/usr/local/anaconda/3.8/bin/python {scriptsdir}/new_training.py -j {jobid} -R {train_size} -B {test_size} -s {sig} -c {configsdir}/config{config_num}.json -o {outdir}/training_outcomes --seed {seed} "
                    ...
                    cmd_delta=f"/usr/local/anaconda/3.8/bin/python {scriptsdir}/new_training.py -j {jobid} -R {train_size} -B {test_size} -s {sig} -c {configsdir}/config{config_num}.json -o {outdir}/training_outcomes --seed {seed} "
