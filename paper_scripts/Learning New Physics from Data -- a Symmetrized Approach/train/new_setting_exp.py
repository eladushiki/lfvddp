...
def exp(N_Ref,N_Bkg,N_Sig,seed,pois=False,Scale=0,Norm=0,Sig_loc=6.4,Sig_scale=0.16):
    np.random.seed(seed)
    N_Bkg_Pois  = np.random.poisson(lam=N_Ref*np.exp(Norm), size=1)[0] if pois else N_Bkg
    N_Ref_Pois  = np.random.poisson(lam=N_Bkg*np.exp(Norm), size=1)[0] if pois else N_Ref
    N_Sig_Pois = np.random.poisson(lam=N_Sig*np.exp(Norm), size=1)[0] if pois else N_Sig
