...
def prepare_training(datasets,Bkg_Aux_str,Sig_Aux_str,Bkg_Data_str,Sig_Data_str,NR="False",ND="False",*args, **kwargs):
    ...
    print('a_size = ',datasets_dict['Ref'].shape,'b_size = ',datasets_dict['Bkg'].shape,'sig_size = ',datasets_dict['Sig'].shape)
    print("dict created")
...
def exp(N_A,N_B,N_Sig,seed,Scale=0,Norm=0,Sig_loc=6.4,Sig_scale=0.16,N_poiss=True,resonant=True):
    ...
    B = np.random.exponential(scale=np.exp(1*Scale), size=(N_B_Pois, 1))
    A  = np.random.exponential(scale=1., size=(N_A_Pois, 1))
    if resonant:
    ...
    print(f'defs: exp, N_A={N_A},N_B={N_B},N_Sig={N_Sig},seed = {seed},Scale={Scale},Norm={Norm},Sig_loc={Sig_loc},Sig_scale={Sig_scale}, N_poiss = {N_poiss}, resonant = {resonant}')
...
def gauss(N_A,N_B,N_Sig,seed,Scale=0,Norm=0,Sig_loc=6.4,Sig_scale=0.16,N_poiss=True,resonant=True,dim=2):
    ...
    print(f'defs: gauss, N_A={N_A},N_B={N_B},N_Sig={N_Sig},seed = {seed},Scale={Scale},Norm={Norm},Sig_loc={Sig_loc},Sig_scale={Sig_scale}, N_poiss = {N_poiss}, resonant = {resonant}')
    print('A: ',A.shape,' B: ',B.shape, ' Sig: ',Sig.shape)
    return A,B,Sig
...
def physics(A_size, B_size, sig_events, seed, channel='em', signal_types=["ggH_taue","vbfH_taue"], phys_variables='Mcoll', N_poiss=True ,combined_portion=1, binned=False, resolution=0.05, normalization=1e5):
    '''
    ...
    resolution : float
        the resolution to which we round the samples (if binned is True).
    normalization : float, str
        if float - the normalization factor. if str ('min-max' or 'standard') - the type of the normalization.
