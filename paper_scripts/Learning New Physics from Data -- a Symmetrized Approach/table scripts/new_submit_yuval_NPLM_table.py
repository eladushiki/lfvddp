...
runtag="test"
A_sizes=['1/10']#['1/2']#[#['4/5','1/2','100/101','1/16','1/4']#['1/4','1/8','1/16','4/5','1/2','13/30']#['1/2','1/4','1/16','4/5','1/2','4/5','100/101']
B_sizes=['1/10']#['1/2']##['1/15', '1/24','1/5','1/101','1/16','1/4']#['1/4','1/8','1/16','1/15', '1/24','13/30']#['1/2','1/4','1/16','1/15', '1/24','1/5','1/101']
number_of_signals = [0]
walltime = ["25:00:00"]
...
def submit_jobs(outdir,scriptsdir):
    for config_num in ["_2d_gauss"]:
        for sig in number_of_signals:
            #N_jobs = 2000 if sig==0 else 99
            N_jobs = 1000 if sig==0 else 100
            for i in range(len(A_sizes)):
                ...
                for jobid in range(N_jobs):
                    ...
                    cmd_TAU+=f"-t TAU --BDstr Bkg --SDstr Sig --BAstr Ref --smlpdf {pdf}"
                    frame.submit.prepare_submit_file(fsubname,setupLines,[cmd_TAU],shortname=f"sub_{jobid}_{i}_{sig}_{config_num}_TAU_{pdf}",setupATLAS=False)
