...
When detected, some physical attributes of each event are measured, e.g., energy, momentum components, angles. Let each event \( x_i \in \mathbb{R}^k \equiv \{\phi^j_i\}\) be a point in \( k \)-dimensional feature space. Each dataset \(\mathbf{A} \in \mathbb{R}^{\true{N} \times k}\) consists of such events.
...
with $|\meas{A}| = \meas{N}$, the observed number of events.
...
\begin{align*}
    \eta(x) &: \mathbb{R}^k \to [0,1] \equiv \prod_{j=1}^k \eta^j(\phi^j)
\end{align*}

with

\begin{align*}
    \eta^j(\phi^j) &: \mathbb{R} \to [0,1] \equiv e^{\nu^j}\theta^j(\phi^j)
\end{align*}
