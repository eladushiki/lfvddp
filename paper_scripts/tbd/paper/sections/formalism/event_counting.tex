...
Due to limited detector efficiency and selection effects, not all events that occur inside the detector are observed. Let $\eta(\bar{x_i})$ be the probability with which an event is detected, and hence appears in the observed dataset.
...
\text{for } x_i \in \tilde{\mathbf{A}}, \quad w_i := \frac{1}{\theta(x_i)} = e^{\nu^j_m}\frac{1}{\eta^j(x_i)}
\end{equation}
