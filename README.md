...
`TrainConfig` defaults to the legacy binwise-constant nuisance implementation. Set `train__nuisance_is_neural_network` to `true` to use a bounded neural `theta(x)` instead. Its input and output dimensions match the primary signal-shift network, while `train__nuisance_nn_inner_layer_nodes` controls its hidden layer and defaults to `2`.
