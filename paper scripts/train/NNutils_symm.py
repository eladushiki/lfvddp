...
def crossentropy_delta_loss(true, pred):
    delta   = pred[:, 0]
    Laux= pred[:, 1]
    y   = true[:, 0]
    #w   = true[:, 1]
    #return tf.reduce_sum((1-y)*tf.math.log(tf.exp((-2)*(delta))+1)) + tf.reduce_sum(y*tf.math.log(tf.exp((2)*(delta))+1)) - tf.reduce_mean(Laux)
    w = tf.reduce_sum(y)/tf.reduce_sum(1-y)
    return tf.reduce_sum(tf.math.log((w*tf.exp((-2)*(delta[y==0]))+1)/(w+1))) + tf.reduce_sum(tf.math.log(((1/w)*tf.exp((2)*(delta[y==1]))+1)/(1+1/w))) - tf.reduce_mean(Laux)