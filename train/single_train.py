...
def main():
    ...
    parser.add_argument("--config", help="config file", dest="config")
    ...
    train_config = TrainConfig.load_from_files([args.config])
    ...
    epochs = epochs_tau if args.t=='TAU' else epochs_delta if args.t=='delta' else max(epochs_tau,epochs_delta)
    patience = patience_tau if args.t=='TAU' else patience_delta if args.t=='delta' else min(patience_tau,patience_delta)
    ...
    sample = em if sample_str=='em' else em_Mcoll if sample_str=='em_Mcoll' else exp if sample_str=='exp' else 'function'
    ...
    if sample_str=='em_Mcoll':
        N_poiss = config['N_poiss']=="True"
        if binned:
            feature,target = prepare_training(em_Mcoll,Bkg_Aux_str,Sig_Aux_str,Bkg_Data_str,Sig_Data_str,train_size = train_size,test_size = test_size, sig_events = sig_events,seed = seed,N_poiss = N_poiss,combined_portion=CP,binned=True,resolution=resolution)
        else:
            feature,target = prepare_training(em_Mcoll,Bkg_Aux_str,Sig_Aux_str,Bkg_Data_str,Sig_Data_str,train_size = train_size,test_size = test_size, sig_events = sig_events,seed = seed,N_poiss = N_poiss,combined_portion=CP)
    if sample_str=='exp':
