...
\[
\log\mathcal{L}_{\reco{P}}(f) \propto -N_\reco{P} + \sum_{\event\in\reco{P}} w_i \log\Big(\alpha \tfrac{1}{Z_\reco{A}}e^{f(\event)} + (1-\alpha)\tfrac{1}{Z_\reco{B}}\Big).
\]
...
\begin{enumerate}
    \item A term proportional to \(\int \alpha (\rho e^{f}-1) \refdist dx\), which, after replacing integrals with weighted sums, yields \(-\alpha\sum_{\reco{P}} w_i (e^{f(\event)}-1)\) up to multiplicative constants and absorbed \(\rho\) factors. The appearance of \(e^{f}-1\) is a consequence of subtracting the baseline intensity.
    \item A sum over events in sample A giving \(\sum_{\event \in \reco{A}} w_i f(\event)\) originating from the \(\sum\log e^{f}=\sum f\) contribution of labeled-A events when comparing labeled likelihood terms.
\end{enumerate}
...
To learn the optimal \( f(x) \) under the constraints stated so far, we train a classifier using the following efficiency-weighted loss:
\[
